...
raw %>%
  group_by(byear) %>%
  summarize(avg_age = mean(death_age)) %>%
  arrange(avg_age)
...
```{r}
x <- raw %>%
  mutate(death_date = paste(dmonth, 01, dyear, sep = "-") %>% mdy()) %>%
...
s3 <- x %>%
  group_by(death_date) %>%
  summarize(total = n(),
            avg_death_age = mean(death_age)) %>%
  mutate(temp = ifelse(month(death_date) %in% c(1, 2, 3, 10, 11, 12), "Cold", "Warm"))

s3 %>%
  ggplot(aes(x = death_date, y = total, color = temp)) +
  geom_line() +
  theme_classic() +
  labs(
    title = "Number of Deaths Over Time",
    subtitle = "More deaths in colder months",
